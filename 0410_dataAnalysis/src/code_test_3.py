...
class DATA_MONITOR:
    ...
    def __init__(self, fname):
        ...
        #print(self.df)
        for i in range(12):
            self.dfint = self.dfint.apply(lambda col: col.map(lambda x: rotar_hexadecimal(x, i)))
#            print(i)
#            print(self.dfint)
            fig = plt.figure(figsize=(10, 6))
            ax = fig.add_subplot(111)
#            self.dfint[8].plot(figsize=(10, 6), title='Plot del Canal', marker='.', linestyle='None')
            self.dfint[8].plot(figsize=(10, 6), title='Plot del Canal')
            plt.xlabel('Índice')
            plt.ylabel('Valor')
            plt.grid()
        plt.show()
    ...
    #         ax.cla()
    #
    #     self.dfint.tail(self.NPTS).plot(ax=axsWF, title=self.fname, subplots=True,
    #                                     ylim=(self.YMIN, self.YMAX))  # ,sharey=True)#,layout=(2,4))
    #
    ...
    #             ax.cla()
    #
    #     self.dfint.tail(self.NPTS).plot(ax=axsH, title=self.fname, kind='hist',
    #                                     bins=range(self.YMIN, self.YMAX, self.BINW), subplots=True,
    #                                     xlim=(self.YMIN, self.YMAX))  # ,sharex=True)
