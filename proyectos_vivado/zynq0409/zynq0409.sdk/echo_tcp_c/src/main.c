...
int main() {
	...
	netif_set_up(echo_netif); /* specify that the network if is up */
			...
			dhcp_timoutcntr = DHC_TIMEOUT;  // ⬅️ aumentamos el tiempo de espera

			while(((echo_netif->ip_addr.addr) == 0) && (dhcp_timoutcntr > 0)) {
//				#ifdef DEBUG
//				if (dhcp_timoutcntr % 100 == 0) {
//					xil_printf("Esperando DHCP... (%d)\n\r", dhcp_timoutcntr);
//				}
//				#endif
				xemacif_input(echo_netif);
				// usleep(1000);
				//dhcp_timoutcntr--; // que raro, al comentar esto si bien el valor no cambia nunca, eventualmente se conecta el dhcp
				// en base al caso minimo, donde no habria ninguna forma
			}
			...
			if (dhcp_timoutcntr <= 0) {
				if ((echo_netif->ip_addr.addr) == 0) {
					...
					IP4_ADDR(&(echo_netif->ip_addr),  192, 168,   1, 123);
					IP4_ADDR(&(echo_netif->netmask), 255, 255, 255,  0);
					IP4_ADDR(&(echo_netif->gw),      192, 168,   1, 5);
				}
			}
	...
	// 0415 testing del ring buffer
	//debug_test_ring_buffer();
	...
	while (1) {
		...
		// === importante ===
		// condiicon que afecta si el buffer se sigue llenando o no
//		if (line_rb.count < RING_BUFFER_CAPACITY) {
//		#ifdef DEBUG
//				xil_printf("[main] check de condiciones\n");
//				xil_printf("\t[main] line counts - ringBCap = %d \n", (int)(line_rb.count - RING_BUFFER_CAPACITY));
//				xil_printf("\t[main] data available: %d \n", is_more_data_available());
//
//		#endif
//		}

		// xd mi error era este is_more_data_available debe ser == 1 por True
		if (line_rb.count < RING_BUFFER_CAPACITY && (is_more_data_available()==1)) {
		    int cargados = load_lines_to_buffer(&line_rb);
		    if (cargados > 0) {
		        xil_printf("[main] Se cargaron %d bloques al ring buffer\n\r", cargados);
		    }
		}
