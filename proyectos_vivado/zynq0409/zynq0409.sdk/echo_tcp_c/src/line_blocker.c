...
int load_lines_to_buffer(LineRingBuffer *rb) {
    ...
    while (rb->count < RING_BUFFER_CAPACITY) {
        ...
        while (lines_added < LINES_PER_BLOCK && *read_ptr != '\0') {
...
#ifdef DEBUG
            xil_printf("=======loading buffer line======\n\r");
            ...
            // esto simula hacer recorrido en memoria o en el tiempo, pues tengo los valores hardcoded
            read_ptr = newline + 1;
            lines_added++;
        }
...
#ifdef DEBUG
	xil_printf("\t---llamada read_next_block_from_buffer---");
	xil_printf("\t\t rb->tail: %d", rb->tail);
	xil_printf("\t\t rb->count: %d", rb->count);
